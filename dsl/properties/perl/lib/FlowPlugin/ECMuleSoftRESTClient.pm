...
package FlowPlugin::ECMuleSoftRESTClient;
use strict;
use warnings;
use LWP::UserAgent;
use JSON;
...
use FlowPDF::Log;
## DO NOT EDIT THIS BLOCK === rest client imports ends, checksum: d4a16d8434f55afdc7e7196f71d935a8 ===
# Place for the custom user imports, e.g. use File::Spec
## DO NOT EDIT THIS BLOCK === rest client starts ===
=head1

FlowPlugin::ECMuleSoftRESTClient->new('http://endpoint', {
...
Use this method to create a new FlowPlugin::ECMuleSoftRESTClient, e.g.

    my $client = FlowPlugin::ECMuleSoftRESTClient->new($endpoint,
...
sub getOrganizationId {
    ...
    $self->{method} = 'getOrganizationId';
    $self->{methodParameters} = \%params;

    my $uri = "/accounts/api/me";
...
sub getEnvironments {
    ...
    $self->{method} = 'getEnvironments';
    $self->{methodParameters} = \%params;

    my $uri = "/accounts/api/organizations/{{organizationId}}/environments";
    logDebug "URI Template: $uri";
    $uri = renderOneLineTemplate($uri, %params);
    logDebug "Rendered URI: $uri";
...
sub getApplications {
    ...
    $self->{method} = 'getApplications';
    $self->{methodParameters} = \%params;

    my $uri = "/cloudhub/api/applications";
    logDebug "URI Template: $uri";
    $uri = renderOneLineTemplate($uri, %params);
    logDebug "Rendered URI: $uri";
...
sub getApplication {
    ...
    $self->{method} = 'getApplication';
    $self->{methodParameters} = \%params;

    my $uri = "/cloudhub/api/applications/{{domain}}";
    logDebug "URI Template: $uri";
    $uri = renderOneLineTemplate($uri, %params);
    logDebug "Rendered URI: $uri";
...
# persistentQueues: in body

# multitenanted: in body
...
sub getAccessToken {
    ...
    my $url = URI->new("https://anypoint.mulesoft.com/accounts/login");
    my %data = ( "username" => $params{ 'username'},
        "password" => $self->{bearerAuth}->{token});
    ...
    if ($response->is_success) {
        ...
        $self->{bearerAuth}->{token} = $json->{'access_token'};
    }
    else {
...
sub augmentRequest {
    ...
    if ($self->method eq 'createApplication') {
...
sub _createApplicationAugmentRequest{
    my ($self, $request, $params) = @_;
    $request->header('X-ANYPNT-ENV-ID', $params->{environmentID});
    return $request;
}

sub _getApplicationsAugmentRequest{
    my ($self, $request, $params) = @_;
    $request->header('X-ANYPNT-ENV-ID', $params->{environmentID});
    return $request;
}

sub _getApplicationAugmentRequest{
    my ($self, $request, $params) = @_;
    $request->header('X-ANYPNT-ENV-ID', $params->{environmentID});
    return $request;
}
